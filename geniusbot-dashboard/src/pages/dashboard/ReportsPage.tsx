      ...
      <form className="reports-filters" onSubmit={(event) => { event.preventDefault(); apply() }}>
        <div className="reports-filters__grid">
          ...
          {isBranchManager ? <div className="reports-fixed-scope"><strong>Branch scope</strong><br />{stringValue(fixedBranch ?? { id: '' }, 'name') || 'Assigned branch'}</div> : <>
            <label>City<select value={draft.city ?? ''} onChange={(event) => setDraft({ ...draft, city: event.target.value || undefined, branchId: undefined })}><option value="">All cities</option>{cities.map((city) => <option key={city} data-i18n-ignore>{city}</option>)}</select></label>
            <label>Branch<select value={draft.branchId ?? ''} onChange={(event) => setDraft({ ...draft, branchId: event.target.value || undefined })}><option value="">All branches</option>{visibleBranches.map((item) => <option key={item.id} value={item.id} data-i18n-ignore>{stringValue(item, 'name')}</option>)}</select></label>
          </>}
          <label>Service<select value={draft.serviceId ?? ''} onChange={(event) => setDraft({ ...draft, serviceId: event.target.value || undefined })}><option value="">All services</option>{services.map((item) => <option key={item.id} value={item.id} data-i18n-ignore>{stringValue(item, 'name')}</option>)}</select></label>
          <label>Doctor<select value={draft.doctorId ?? ''} onChange={(event) => setDraft({ ...draft, doctorId: event.target.value || undefined })}><option value="">All doctors</option>{doctors.map((item) => <option key={item.id} value={item.id} data-i18n-ignore>{stringValue(item, 'full_name')}</option>)}</select></label>
