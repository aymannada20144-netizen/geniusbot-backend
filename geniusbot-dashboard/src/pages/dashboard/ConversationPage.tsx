...
export function ConversationPage() {
  ...
  return <section className="conversation-page">
    ...
    <header className="conversation-header"><div><h2>{data.patient.fullName}</h2><p>{data.patient.phoneNumber}</p></div><div className="conversation-controls"><span className={`handling-badge handling-badge--${badge}`}>{label}</span>{data.ownership === 'HUMAN_HANDLING' ? <Button size="sm" disabled={busy} onClick={() => changeOwnership('return-to-shaden')}>Return to AI assistant</Button> : <Button size="sm" disabled={busy} onClick={() => changeOwnership('takeover')}>{data.conversation ? 'Take Over' : 'Start Human Conversation'}</Button>}</div></header>
    {error && <p className="conversation-error" role="alert">{error}</p>}
    <div className="messages">{data.messages.length === 0 ? <div className="conversation-empty"><strong>{data.conversation ? 'No messages yet.' : 'No conversation yet.'}</strong><span>{data.conversation ? 'New messages will appear here.' : 'Start human handling to contact this patient.'}</span></div> : data.messages.map((message) => <article className={`message message--${message.senderType}`} key={message.id}><strong>{message.senderType === 'patient' ? 'Patient' : message.senderType === 'bot' ? 'AI assistant' : message.senderType === 'staff' ? 'Human staff' : 'System'}</strong><p>{message.messageText}</p><time>{new Date(message.createdAt).toLocaleString()}</time></article>)}</div>
