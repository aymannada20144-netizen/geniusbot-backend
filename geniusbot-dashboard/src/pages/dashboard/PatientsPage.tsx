...
import { createPatient, getPatient, getPatientAppointments, getPatients, setPatientActive, updatePatient } from '../../api/patientsApi'
...
export function PatientsPage() {
  ...
  const filtered = useMemo(() => patients.filter((patient) => {
    ...
    return matches && (handling === 'all' || mode === handling)
  }), [patients, search, handling])
  const now = new Date()
  const stats = {
  ...
  return <section className="patients-page">
    ...
    {(selectedId || creating) && <div className="patient-modal" onMouseDown={(event) => event.target === event.currentTarget && close()}><div className="patient-dialog" role="dialog" aria-modal="true" aria-labelledby="patient-dialog-title">
      ...
      {!editing && patientQuery.data && <div className="patient-details">
        ...
        <div className="patient-dialog__actions"><button onClick={beginEdit}>Edit</button><button disabled={status.isPending} onClick={() => status.mutate({ id: patientQuery.data!.id, active: !patientQuery.data!.is_active })}>{patientQuery.data.is_active ? 'Deactivate' : 'Reactivate'}</button><Link className="patients-open" to={`/dashboard/patients/${patientQuery.data.id}/conversation`}>Open Conversation</Link></div>
      </div>}
      {editing && <form onSubmit={submit}><div className="patient-form">
        ...
        <label>Phone *<input dir="ltr" required aria-invalid={Boolean(phoneErrors.phone)} placeholder="+9665XXXXXXXX" value={form.phone_number ?? ''} onChange={(e) => { setForm({ ...form, phone_number: e.target.value }); setPhoneErrors({ ...phoneErrors, phone: undefined }) }} />{phoneErrors.phone && <small className="patient-form-error">{phoneErrors.phone}</small>}</label>
