...
export function PricesPage() {
  ...
  return <section className="prices-page">
    ...
    <div className="prices-page__filters" aria-label="Price filters">
      ...
      <button className="action-button action-button--refresh" type="button" disabled={prices.isFetching} onClick={() => prices.refetch()}>Refresh</button>
    </div>
    ...
    {rows.length > 0 && <div className="prices-page__table"><table><thead><tr><th>Service</th><th className="prices-page__payment-column">Payment method</th><th>Insurance company</th><th>Insurance class</th><th>Price</th><th>Currency</th><th>Valid from</th><th>Valid to</th><th>Status</th><th className="prices-page__actions-column">Actions</th></tr></thead><tbody>
      {rows.map((price) => <tr key={price.id}><td>{price.service_name ?? label(services, price.service_id, 'name')}</td><td className="prices-page__payment-column">{price.payment_method_name ?? label(methods, price.payment_method_id, 'name')}</td><td>{price.insurance_company_id ? price.insurance_company_name ?? label(companies, price.insurance_company_id, 'name') : '—'}</td><td>{price.insurance_class_id ? price.insurance_class_name ?? label(classes, price.insurance_class_id, 'class_name') : '—'}</td><td>{Number(price.price).toFixed(2)}</td><td>{price.currency}</td><td>{price.valid_from.slice(0, 10)}</td><td>{price.valid_to?.slice(0, 10) ?? 'Open-ended'}</td><td><span className={`prices-page__status prices-page__status--${price.is_active ? 'active' : 'inactive'}`}>{price.is_active ? 'Active' : 'Inactive'}</span></td><td className="prices-page__actions"><button className="action-button action-button--edit" type="button" onClick={() => open(price)}>{canUpdate ? 'Edit' : 'View'}</button>{canUpdate && <button className="action-button action-button--status" type="button" disabled={status.isPending} onClick={() => window.confirm(`${price.is_active ? 'Deactivate' : 'Activate'} this price?`) && status.mutate({ id: price.id, active: !price.is_active })}>{price.is_active ? 'Deactivate' : 'Activate'}</button>}</td></tr>)}
    </tbody></table></div>}
