...
function recognizeCourtesy(text) { if (/^(?:الله )?يعطيك العافيه$/.test(text)) return { type: 'courtesy', kind: 'wellbeing' }; if (/حبيبتي/.test(text)) return { type: 'courtesy', kind: 'affection' }; if (/^(?:تسلمي|ما قصرتي)(?: حبيبتي)?$/.test(text)) return { type: 'courtesy', kind: 'praise' }; if (/^(?:اسمك (?:جميل|حلو)(?: يا [\p{L}]+)?|[\p{L}]+ اسمك (?:جميل|حلو)|ما شا(?:ء)? الله اسم(?:ك)? (?:جميل|حلو))$/u.test(text)) return { type: 'courtesy', kind: 'praise' }; if (/^(?:شكرا(?: لك)?|متشكره|مشكوره)$/.test(text)) return { type: 'courtesy', kind: 'thanks' }; return null; }
